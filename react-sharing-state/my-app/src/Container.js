...
export default function Container({ items }) {
  ...
  return (
    <div>
      ...
      <div>
        <Button text="Prev" onClick={() => setActiveIndex(((activeIndex - 1) + items.length) % items.length)} />
        <Buttons count={items.length} activeIndex={activeIndex} setActiveIndex={setActiveIndex} />
        <Button text="Next" onClick={() => setActiveIndex((activeIndex + 1) % items.length)} />
      </div>
    </div>
  );
}
 ...
 */
function Button({ key, text, onClick, backgroundColor='white' }) {
 ...
 */
function Buttons({ count, activeIndex, setActiveIndex}) {
  const buttons = [];
