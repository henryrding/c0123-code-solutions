...
export function useTodos() {
  ...
  async function toggleCompleted(todoId) {
     ...
     * Note that it is critical that you pass a _new_ array. Do not modify the `todos` array.
     */
    const oldTodoCopy = todos.find((todo) => todo.todoId === todoId);
    oldTodoCopy.isCompleted = !oldTodoCopy.isCompleted;
    try {
      const updatedTodo = await updateTodo(oldTodoCopy);
      const allTodos = todos.map((originalTodo) =>
        originalTodo.todoId === updatedTodo.todoId
          ? updatedTodo
          : originalTodo
      );
      setTodos(allTodos);
